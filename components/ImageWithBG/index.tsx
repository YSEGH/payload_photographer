...
const index = ({ photo }) => {
  ...
  const onClickImageHandler = (e) => {
    if (isActive) {
      if (e.target === overlay.current || e.target === close.current) {
        setIsActive(false);
      }
      return;
    }
  ...
  return (
    <div
      ...
      className={cx(style.overlay, {
        [style["overlay--active"]]: isActive,
      })}
      onClick={onClickImageHandler}
    >
      <div
        ref={close}
        className={cx(style.close, { [style["close--active"]]: isActive })}
      >
        <svg
          width="24"
          height="24"
      ...
      <div
        className={cx(style.infos, { [style["infos--active"]]: isActive })}
      ...
      <img
        ref={image}
        onClick={onClickImageHandler}
        className={cx(global.masonry__img, style.imgWithBg, {
          [style["imgWithBg--active"]]: isActive,
        })}
        src={photo.photo.url}
        alt={photo.photo.alt}
      />
      <div
        ref={caption}
        className={cx(style.caption, {
          [style["caption--active"]]: isActive && showInfos,
        })}
      >
        <svg
          width="24"
